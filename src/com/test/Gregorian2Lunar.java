...
public class Gregorian2Lunar {
	 ...
	 */
	public Gregorian2Lunar(Calendar cal) {
	...
	/**
	 * 计算未来和你生日的阳历和阴历完全重合的那天
	 * 
	 * @throws ParseException
	 */
	private static void gregorianLunarEqualsBirth() throws ParseException {
		Calendar today = Calendar.getInstance();
		int year = today.getWeekYear();
		while (true) {
			today.setTime(chineseDateFormat.parse(year + "-" + gregorian)); // 此处输入公历的出生日期
			Gregorian2Lunar gregorian2LunarTemp = new Gregorian2Lunar(today);
			String gregorianTemp = chineseDateFormat.format(today.getTime());
			String lunarTemp = gregorian2LunarTemp + "";
			gregorianTemp = gregorianTemp.substring(gregorianTemp.indexOf("-") + 1);
			lunarTemp = lunarTemp.substring(lunarTemp.indexOf("年") + 1);
			if (gregorianTemp.equals(gregorian) && lunarTemp.equals(lunar)) {
				System.out.println("\n未来和你生日的阳历和阴历完全重合的那天是：\n\t公历：" + year + "-" + gregorianTemp + "\t农历：" + year + "年" + lunarTemp);
				return;
			}
			++year;
		}
	...
	public static void main(String[] args) throws ParseException {
		Calendar today = Calendar.getInstance();
		today.setTime(chineseDateFormat.parse("1993-08-28")); // 此处输入公历的出生日期
		Gregorian2Lunar gregorian2Lunar = new Gregorian2Lunar(today);
		gregorian = chineseDateFormat.format(today.getTime());
		lunar = gregorian2Lunar + "";
		System.out.println("公历生日：" + gregorian + "\t\t农历生日：" + lunar);
		gregorian = gregorian.substring(gregorian.indexOf("-") + 1);
		lunar = lunar.substring(lunar.indexOf("年") + 1);
		gregorianLunarEqualsBirth();
	}
}
