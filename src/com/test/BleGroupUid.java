...
/**
 * 蓝牙设备组ID的生成器 Created by MJCoder on 2018-05-21.
 */

public class BleGroupUid {

	public static void main(String[] args) {
		System.out.println(getBleGroupUid("fdasfafdasfsff", "fdsdsafuyrafsaf"));
	 ...
	 * @return
	 */
	public static String getBleGroupUid(String mainBleMacAddress, String followBleMacAddress) {
		// 通过时间戳（以秒为单位）
		// String temp = Integer.toHexString((int) (System.currentTimeMillis() /
		// 1000)).toUpperCase();
		String temp = getCode(mainBleMacAddress.replaceAll(":", "")) + getCode(followBleMacAddress.replaceAll(":", ""));
		String result = ":";
	...
	private static int GetCrc16(final byte[] pData) {
	...
	private static String getCode(String data) {
		if (data != null && data.length() >= 4) {
			int crc16 = GetCrc16(HexUtil.hexStringToBytes(data));
			byte[] result = int2byte(crc16);
			String code = HexUtil.formatHexString(result);
			return code;
		} else {
			return null;
		}
